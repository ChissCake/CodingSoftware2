...
public final class SequencePalindrome {
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
         ...
         *
         * for (int i = 0; i < s.length() / 2; i++) { //.entry(0)
         * .entry(s.length() - i) int one = s.entry(i); int two =
         * s.entry(s.length() - i - 1); if (one != two) { palindromeFlag =
         * false; } }
         *
        ...
        if (s.length() > 1) {
            int one, two;
            one = s.remove(0);
            two = s.remove(s.length() - 1);

            flag = (one == two && isPalindrome(s));
